...
class BBCode_Api_User extends Zikula_Api
{
    ...
    */
    public function transform($args)
    {
        ...
        if($this->getVar('lightbox_enabled') == true) {
        ...
        if (is_array($args['extrainfo'])) {
          foreach ($args['extrainfo'] as $text) {
            $result[] = $this->transformString($text);
          }
        } else {
          $result = $this->transformString($args['extrainfo']);
        }
    ...
    */
    public function transformString($message)
    {
        ...
        $message = preg_replace_callback(
                "#\[url\]([a-z]+?://){1}(.*?)\[/url\]#si",
                Array($this, 'linktest_callback_0'),
                $message);
        ...
        $message = preg_replace_callback(
                "#\[url\](.*?)\[/url\]#si",
                Array($this, 'linktest_callback_1'),
                $message);

        // [url=xxxx://www.phpbb.com]phpBB[/url] code..
        $message = preg_replace_callback(
                "#\[url=([a-z]+?://){1}(.*?)\](.*?)\[/url\]#si",
                Array($this, 'linktest_callback_2'),
                $message);

        // [url=www.phpbb.com]phpBB[/url] code.. (no xxxx:// prefix).
        $message = preg_replace_callback(
                "#\[url=(.*?)\](.*?)\[/url\]#si",
                Array($this, 'linktest_callback_3'),
                $message);

        // [email]user@domain.tld[/email] code..
        $message = preg_replace_callback(
                "#\[email\](.*?)\[/email\]#si",
                Array($this, 'linktest_callback_4'),
                $message);
    ...
    */
    public function minimize_displayurl($displayurl)
    {
        ...
        $maxsize = $this->getVar('link_shrinksize');
